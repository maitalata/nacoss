...
$institution= $from= $to= $certificate= $response= "";

if($_SERVER['REQUEST_METHOD'] == 'POST'){
	$data_check = new DataValidation();
	
	$institution = $data_check->validate($_POST['institution']);
	$from = $data_check->validate($_POST['from']);
	$to = $data_check->validate($_POST['to']);
	$certificate = $data_check->validate($_POST['certificate']);
	
	
	if($data_check->emptyCheck($institution, $from, $to, $certificate)){
		if($db->query("UPDATE institutions SET institutionName='$institution', fromD='$from', toD='$to', certificate='$certificate' WHERE id='$id'")){
			header("Location: institutions.php");
		}else{
...
$query = $db->query("SELECT * FROM institutions WHERE id='$id'");
...
<?php echo $response; ?>
	...
	<form action="" method="POST">
		<div class="w3-section">      
			<input class="w3-input" type="text" name="institution" placeholder="Institution Name" value="<?php echo $row['institutionName']; ?>" >
		</div>
		<div class="w3-section">
			<select class="w3-select" name="from">
				...
				<option value='2019' <?php echo $row['fromD'] == "2019"?"selected":""; ?> >2019</option>
				<option value='2020' <?php echo $row['fromD'] == "2020"?"selected":""; ?> >2020</option>
			</select>
		</div>
		<div class="w3-section">
			<select class="w3-select" name="to">
				...
				<option value='2019' <?php echo $row['toD'] == "2019"?"selected":""; ?> >2019</option>
				<option value='2020' <?php echo $row['toD'] == "2020"?"selected":""; ?> >2020</option>
			</select>
		</div>
		<div class="w3-section">      
			<input class="w3-input" type="text" name="certificate" placeholder="Certificate Obtained" value="<?php echo $row['certificate']; ?>" >
		</div>
